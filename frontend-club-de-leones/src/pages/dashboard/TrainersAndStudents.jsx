...
function TrainersAndStudents() {
  ...
  return (
    <div>
      ...
      <Dialog header={`${isEdit ? 'Editar' : 'Agregar'} alumno`} visible={showDialog} className='w-full md:w-1/3 lg:w-1/4' onHide={() => setShowDialog(false)}>
          <div className='flex flex-col gap-3 my-3'>
            <InputText value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Título" className="w-full md:w-14rem" style={{width: '100%', display: 'flex'}} />
            <Dropdown disabled={isEdit ? true : false} value={id} onChange={(e) => setId(e.value)} options={allUsers} optionLabel="full_name"  optionValue='id'
                optionGroupLabel="name" optionGroupChildren="users" optionGroupTemplate={groupedItemTemplate} className="w-full md:w-14rem" placeholder="Selecciona el alumno" style={{width: '100%', display: 'flex'}} />
